...
import { IsLogin } from '../Utils/AuthCheck';
    ...
    const submit = async (e) => {
        ...
        firebase.auth().createUserWithEmailAndPassword(email, password)
            .then((userCredential) => {
                ...
                db.collection("users").doc(user._delegate.uid).set({
                    email: email,
                ...
                db.collection(`users/${user._delegate.uid}/myusers`).add({});
        ...
        <>
            <div className="lg:flex">
                ...
                <div className="lg:w-1/2 xl:max-w-screen-sm">
                    {
                        loading ? <div className='flex justify-center mt-80'><Spinner /></div> : <div>
                            <div className="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
                                <div className="cursor-pointer flex items-center">
                                    <div>

                                    </div>
                                    <div className="text-4xl font-bold text-indigo-800 tracking-wide ml-2 font-semibold">FireStore Cloud Chat App</div>
                                </div>
                            </div>
                                ...
                                <div className="mt-12">
                                    <form className='space-y-6'>
                                        <div>
                                            ...
                                            <input value={email} onChange={(e) => setEmail(e.target.value)} className="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type="" placeholder="Email Address" />
                                        </div>
                                        <div>
                                            {/* <div className="text-sm font-bold text-gray-700 tracking-wide">Name</div> */}
                                            <input value={name} onChange={(e) => setName(e.target.value)} className="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type="" placeholder="Full Name" />
                                        </div>
                                            ...
                                            <input value={password} onChange={(e) => setPassword(e.target.value)} className="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type="" placeholder="Enter your password" />
                                     
                                        <div className="mt-10">
                                            <button onClick={submit} className="bg-indigo-500 text-gray-100 p-4 w-full rounded-full tracking-wide
